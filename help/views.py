...
def homepage(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            if text_is_spam(form.cleaned_data['body'], request):
                ...
                return render_to_response('public_spam.html', RequestContext(request, {}))
            else:
                ticket = form.save()
                return HttpResponseRedirect('%s?ticket=%s&email=%s'% (
                    reverse('help_public_view'),
    ...
    return render_to_response('public_homepage.html',
...
def view_ticket(request):
    ...
    if ticket_req and email:
        ...
        if ticket:
            ...
            return render_to_response('public_view_ticket.html',
                RequestContext(request, {
                    'ticket': ticket,
                    'helpdesk_settings': helpdesk_settings,
                    'next': redirect_url,
    ...
    return render_to_response('public_view_form.html',
        RequestContext(request, {
            'ticket': ticket,
            'email': email,
...
@login_required
def view_ticket_detail(request, ticket):
...
class TicketsFilterCustomer(django_filters.FilterSet):
...
def view_all_ticket(request):
    ...
    tickets = Ticket.objects.filter(submitter_email__iexact=user_email)
    filters = TicketsFilterCustomer(request.GET, queryset=Ticket.objects.filter(submitter_email__iexact=user_email)) #creamos el filtro en base al usuario actual
...
def change_language(request):
    ...
    return render_to_response('public_change_language.html',
